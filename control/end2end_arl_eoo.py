...
__all__ = ['EndToEndARLEOO']


class EndToEndARLEOO(BaseClass):
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.current_epoch >= self.hparams.control_epoch:
            ...
            if self.hparams.loss_type['adversary'] == 'Classification':
                loss_ctl = self.criterion['adversary'](s_hat[~mask].squeeze(), s[~mask].squeeze()) +  self.criterion['adversary'](s_hat[mask].squeeze(), s[mask].squeeze())
            else:
                loss_ctl = self.criterion['adversary'](s_hat[~mask].squeeze(), s[~mask].float().squeeze()) + self.criterion['adversary'](s_hat[mask].squeeze(), s[mask].float().squeeze())
        else:
