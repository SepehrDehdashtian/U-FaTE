...
__all__ = ['EndToEndEntropy']


class EndToEndEntropy(BaseClass):
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.current_epoch >= self.hparams.control_epoch:
            ...
            # Passing uniform distribution as the label to adversary branch
            uniform_label = torch.ones(*s.shape, device=s.device)
            loss_ctl = self.criterion['adversary'](s_hat, uniform_label.long())
        else:
        ...
        if self.current_epoch >= self.hparams.control_epoch:
            for _ in range(self.hparams.num_adv_train_iters):
                ...
                # Passing uniform distribution as the label to adversary branch
                # uniform_label = torch.ones(*s.shape, device=s.device)
                loss_ctl = self.criterion['adversary'](s_hat, s.long())
