...
__all__ = ['FairFaceImageLoaderVal']
...
class PrepareFairFace:
    ...
    def load_data(self):
        ...
        # Train on both validation and train splits to get the upper bound of the trade-off
        train_images = np.concatenate((train_images, val_images), axis=0)
        train_y = torch.cat((train_y, val_y), axis=0)
        train_s = torch.cat((train_s, val_s), axis=0)
...
class FairFaceImageLoaderVal(pl.LightningDataModule):
    def __init__(self, opts):
        super().__init__()
        self.opts = opts
        if opts.ngpu == 0:
