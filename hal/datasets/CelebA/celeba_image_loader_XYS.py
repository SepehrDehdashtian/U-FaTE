...
class PrepareCelebA:
    ...
    def load_data(self):
        ...
        # pool = Pool(self.opts.dataset_options['dataset']['numCores'])

        print("Loading images ...")
        tic = time.time()

        # images = list(pool.imap(self.image_loader, tqdm(csv_data["image_id"].values)))
        # images = list()
        # for img in tqdm(pool.imap(self.image_loader, csv_data["image_id"].values)):
        #     images.append(img)
        # pool.close()
        # pool.join()
        ...
        if not self.opts.load_all:
            ...
            # images = csv_data["image_id"].values[:2000]
        else:
            # images = list(map(self.image_loader, tqdm(csv_data["image_id"].values[:182637])))
            images = list(map(self.image_loader, tqdm(csv_data["image_id"].values)))
            # images = list(map(self.image_loader, csv_data["image_id"].values[:2000]))
        ...
        # data['train'] = {'imgs': images[:5001],          'y': y[:5001],         's': s[:5001]}
        # data['val']   = {'imgs': images[162770:163770],    'y': y[162770:163770],   's': s[162770:163770]}
        # data['test']  = {'imgs': images[182637:183637],          'y': y[182637:183637],         's': s[182637:183637]}

        # import pdb; pdb.set_trace()

        # data['train'] = {'imgs': images[:1000],          'y': y[:1000],         's': s[:1000]}
        # data['val']   = {'imgs': images[1000:1500],    'y': y[1000:1500],   's': s[1000:1500]}
        # data['test']  = {'imgs': images[1500:],          'y': y[1500:],         's': s[1500:]}
...
class CelebAImageLoaderXYS(pl.LightningDataModule):
    def __init__(self, opts):
        super().__init__()
        self.opts = opts
        if opts.ngpu == 0:
    ...
    def train_kernel_dataloader(self):
        ...
        if not ('uint8' in self.opts.dataset_options['dataset'].keys() and self.opts.dataset_options['dataset']['uint8']):
            if self.opts.load_all:
                ...
                # images = list(map(self.image_loader, tqdm(self.data['train']['imgs'][idx_sampled], leave=False)))
