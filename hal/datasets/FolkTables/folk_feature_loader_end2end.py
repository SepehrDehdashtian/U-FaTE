...
__all__ = ['FolkTablesFeatureLoaderE2E']
...
class PrepareFolkTables:
    ...
    def load_data(self):
        ...
        features = torch.from_numpy(np.loadtxt(os.path.join(dataset_path, 'features_embedded.out')))
        labels = torch.from_numpy(np.loadtxt(os.path.join(dataset_path, 'label.out')))
        # features = torch.from_numpy(np.loadtxt(os.path.join(dataset_path, 'emb_features.out')))
        # labels = torch.from_numpy(np.loadtxt(os.path.join(dataset_path, 'emb_label.out')))
        if sensitive_attr.lower() == 'race':
            ...
            # sensitive = torch.from_numpy(np.loadtxt(os.path.join(dataset_path, 'emb_group.out'))).unsqueeze(1)
...
class FolkTablesFeatureLoaderE2E(pl.LightningDataModule):
    def __init__(self, opts):
        super().__init__()
        self.opts = opts
        if opts.ngpu == 0:
