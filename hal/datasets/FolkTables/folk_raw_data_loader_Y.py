...
class PrepareFolkTables:
    ...
    def load_data(self):
        ...
        # X and Y and S
        data = dict()
        data['train']  = {'x': labels[idx_train].unsqueeze(-1),                 'y': labels[idx_train], 's': sensitive[idx_train]}
        data['val']  = {'x': labels[idx_val.numpy().tolist()].unsqueeze(-1),    'y': labels[idx_val],   's': sensitive[idx_val]}
        data['test']  = {'x': labels[idx_test.numpy().tolist()].unsqueeze(-1),  'y': labels[idx_test],  's': sensitive[idx_test]}
...
class FolkTablesRawLoaderY(pl.LightningDataModule):
    def __init__(self, opts):
        super().__init__()
        self.opts = opts
        if opts.ngpu == 0:
