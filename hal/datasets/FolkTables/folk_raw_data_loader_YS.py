...
class PrepareFolkTables:
    ...
    def load_data(self):
        ...
        # X and Y and S
        data = dict()
        data['train']  = {'x': torch.cat((sensitive[idx_train], labels[idx_train].unsqueeze(-1)), dim=1),                                   'y': labels[idx_train], 's': sensitive[idx_train]}
        data['val']    = {'x': torch.cat((sensitive[idx_val.numpy().tolist()], labels[idx_val.numpy().tolist()].unsqueeze(-1)), dim=1),     'y': labels[idx_val],   's': sensitive[idx_val]}
        data['test']   = {'x': torch.cat((sensitive[idx_test.numpy().tolist()], labels[idx_test.numpy().tolist()].unsqueeze(-1)), dim=1),   'y': labels[idx_test],  's': sensitive[idx_test]}
...
class FolkTablesRawLoaderYS(pl.LightningDataModule):
    def __init__(self, opts):
        super().__init__()
        self.opts = opts
        if opts.ngpu == 0:
